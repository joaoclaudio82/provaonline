...
export const GabaritoView = Object.freeze({
  ...
  _buildEntry(item, index) {
    ...
    entry.innerHTML =
      ...
      `<div style="font-size:16px;margin:4px 0 8px;line-height:1.4">${Utils.escapeHtml(item.q)}</div>` +
      `<div style="font-size:15px;line-height:1.6">${optionsHtml}</div>` +
      `<div class="hint" style="margin-top:6px"><em>${Utils.escapeHtml(item.e)}</em></div>`;
    return entry;
  },
});
